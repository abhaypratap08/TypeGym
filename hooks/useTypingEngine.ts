...
export function useTypingEngine(): TypingEngineState & TypingEngineActions {
  ...
  /** When set, resetTest uses this list instead of buildWordList (multiplayer seeded race). */
  const lockedRaceWordsRef = useRef<string[] | null>(null)
  ...
  const resetTest = useCallback(() => {
    ...
    const locked = lockedRaceWordsRef.current
    const nextWords = locked ?? buildWordList(mode, timeSetting, wordSetting, codeLanguage)
    setWords(nextWords)
    setInput('')
    setResults([])
    setWIdx(0)
    ...
    setElapsed(0)
    setTL(timeSetting)
  }, [clearTimers, mode, timeSetting, wordSetting, codeLanguage])
  ...
  const enterCustomWordRace = useCallback((list: readonly string[]) => {
    if (list.length === 0) return
    lockedRaceWordsRef.current = [...list]
    setModeState('words')
    setWordState(list.length)
  }, [])

  const exitCustomWordRace = useCallback(() => {
    lockedRaceWordsRef.current = null
    setModeState('time')
    setWordState(50)
  }, [])
